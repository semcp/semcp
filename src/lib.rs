...
impl ContainerExecutor {
    pub fn new(docker_image: String, verbose: bool) -> Self {
        ...
        let falco_enabled = security_policy.as_ref().map_or(false, |p| p.spec.falco.enabled);
        ...
        if self.falco_enabled {
            if let Some(policy) = &self.security_policy {
                if let Ok(Some(rule_file_path)) = policy.generate_falco_rule_file() {
                    ...
                    if self.verbose {
                        eprintln!("Falco monitoring enabled with rules: {}", rule_file_path.display());
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_falco_integration() {
        let _runner = NpxRunner::new("node:20".to_string(), false)
            .with_falco(true);
